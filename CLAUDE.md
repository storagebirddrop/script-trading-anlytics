...
**Tabs:**
- **Portfolio tab:** Portfolio Health Bar (oversold%/neutral%/extended%/sentiment), Opportunity panel (top-3 most oversold with signal-strength labels), Risk panel (top-3 most extended), then asset cards with ATR Distance (semantically coloured: green=oversold, orange=extended, red=extreme), inline historical percentile badge (P8%), and VP position badge when data is available; filterable by **timeframe (Daily/Weekly)**, regime, and category. Switching timeframe refreshes all cards, the health bar, and opportunity/risk panels to that timeframe's data; the cross-reference ATR label on each card flips between `W-ATR Dist.` (in daily mode) and `D-ATR Dist.` (in weekly mode). Regime filter resets on timeframe change to avoid stale cross-timeframe filtering. Macro assets are excluded from this tab.
