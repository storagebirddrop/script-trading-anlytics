...
Standalone BTC cycle signals page (not a tab). Linked from main dashboard header via `₿ BTC` link. Loads `assets/btc_signals.json` on page init and renders four signal sections: Price Structure (6 cards), Sentiment & Positioning (6 cards — ETF Net Daily Flows is an active signal card when `SOSOVALUE_API_KEY` is set, locked otherwise), Mining & Liquidity (4 cards — Hash Ribbons, Puell Multiple, Stablecoin Supply, Global M2 with 12-week lag), On-Chain (4 active cards: MVRV Z-Score, NUPL, SOPR, CVDD via Coinmetrics; 3 locked cards: RHODL Ratio, LTH/STH MVRV Cross, Reserve Risk — require Glassnode UTXO age-band data). Global M2 shows "Requires FRED_API_KEY" when absent; ETF Flows shows "Requires SoSoValue API key" when absent; both count in confluence when present. All 4 on-chain cards count in confluence unconditionally (Coinmetrics is free/no-key). Max active signals: ~20 when both optional keys are set. Confluence banner shows accumulate/neutral/distribute counts and a phase badge. All DOM construction is CSP-safe (`createElement`/`textContent`/`element.style` — no `innerHTML` for dynamic data).
