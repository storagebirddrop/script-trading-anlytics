...
Written by `generate_btc_signals_json(history_df, dashboard)` in `calculate_metrics.py`. BTC cycle indicator confluence data for the standalone `btc.html` page. Sections: `price_indicators` (200WMA, 200DMA, Pi Cycle, RSI daily/weekly, ATR regime, VP position), `sentiment` (Fear & Greed, Funding Rate, OI), `market_structure` (BTC dominance, Altseason), `mining` (Hash Ribbons — 30/60DMA of hashrate via mempool.space), `liquidity` (USDT+USDC combined supply and stablecoin dominance via CoinGecko, plus `global_m2_billion_usd`, `m2_12w_lagged_change_pct`, `m2_current_change_pct`, `signal_global_m2` from FRED `WM2NS` when `FRED_API_KEY` env var is set), `confluence` (accumulate/distribute/neutral counts, phase, strength). Path constant: `BTC_SIGNALS_JSON_PATH` in `trading_utils/config.py`. Three fetch functions in `calculate_metrics.py`: `fetch_hash_ribbons()` (mempool.space free API), `fetch_stablecoin_trend()` (CoinGecko free API), `fetch_global_m2()` (FRED free API — requires `FRED_API_KEY` env var; gracefully returns `None` when absent). `FRED_API_KEY` is passed to the "Calculate metrics" CI step via `secrets.FRED_API_KEY`. Copied by `build_dashboard.py`.

**`btc.html` page** — `dashboard/btc.html`, `dashboard/js/btc.js`, `dashboard/css/btc.css`:
Standalone BTC cycle signals page (not a tab). Linked from main dashboard header via `₿ BTC` link. Loads `assets/btc_signals.json` on page init and renders four signal sections: Price Structure (6 cards), Sentiment & Positioning (6 cards — includes ETF Net Daily Flows locked card requiring SoSoValue API key), Mining & Liquidity (3 cards — Hash Ribbons, Stablecoin Supply, Global M2 with 12-week lag), On-Chain (8 locked Glassnode placeholder cards). Global M2 card shows "Requires FRED_API_KEY" when key is absent; counts in confluence when key is present. Confluence banner shows accumulate/neutral/distribute counts and a phase badge. All DOM construction is CSP-safe (`createElement`/`textContent`/`element.style` — no `innerHTML` for dynamic data).
